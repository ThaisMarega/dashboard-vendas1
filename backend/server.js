...
require('dotenv').config(); // carrega variáveis .env em local; no Render ignora sem problema
process.on('unhandledRejection', err => { console.error('UNHANDLED REJECTION:', err); });
process.on('uncaughtException',  err => { console.error('UNCAUGHT EXCEPTION:', err); });
...
// ===================================
// CRIAÇÃO DAS TABELAS (Executar na subida)
// ===================================
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: vendedora.id }, JWT_SECRET, { expiresIn: '24h' });
    res.json({
      token,
      vendedora: {
        id: vendedora.id,
...
// Cadastrar vendedora (rota admin)
app.post('/api/vendedoras', async (req, res) => {
  try {
    ...
    if (error.code === '23505') return res.status(400).json({ erro: 'Vendedora já cadastrada' });
    console.error(error);
    res.status(500).json({ erro: 'Erro no servidor' });
  }
});
...
app.get('/api/vendas', autenticar, async (req, res) => {
  try {
    ...
    if (dataInicio && dataFim) {
      query += ' AND data_venda BETWEEN $2 AND $3';
...
app.post('/api/vendas', autenticar, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO vendas (vendedora_id, valor, forma_pagamento, cliente_nome, observacao) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
app.post('/api/condicionais', autenticar, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO condicionais (vendedora_id, quantidade_pecas, valor_total, cliente_nome, observacao) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
app.get('/api/metas/:data', autenticar, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      const vendedora = await pool.query('SELECT meta_padrao FROM vendedoras WHERE id = $1', [req.vendedoraId]);
      return res.json({ valor_meta: vendedora.rows[0]?.meta_padrao ?? 15000 });
    }
    res.json(result.rows[0]);
  } catch (error) {
    console.error(error);
